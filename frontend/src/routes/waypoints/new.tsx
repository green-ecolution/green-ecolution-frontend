...
function NewWaypoint() {
  ...
  const [lastSelectedTree, setLastSelectedTree] = useState<FakeTree>();
  const [selectedTrees, setSelectedTrees] = useState<FakeTree[]>([]);
  ...
  const onTreeClick = (tree: FakeTree) => {
    setLastSelectedTree(tree);
    mapRef.current?.flyTo([tree.lat, tree.lng]);
  };

  const onTreeRemove = (tree: FakeTree) => {
...
interface NewWaypointControlProps {
  selection: FakeTree[];
  lastSelectedTree?: FakeTree;
  onTreeClick: (tree: FakeTree) => void;
  onTreeRemove: (tree: FakeTree) => void;
}
...
const TreeInfo = ({ tree }: { tree?: FakeTree }) => {
  if (!tree) return <></>;
  return (
    <Card>
      <CardHeader>
