...
const WateringPlanUpdate = ({
  ...
  const { initForm, loadedData } = useInitFormQuery(wateringPlanIdQuery(wateringPlanId), (data) => ({
  ...
  const { register, handleSubmit, formState, onSubmit, isError } = useWateringPlanUpdateForm({
    wateringPlanId,
    onUpdateSuccess,
    onUpdateError,
